...
class GradientNormLoggingCallback(transformers.TrainerCallback):
    def __init__(self):
        self.global_max_allocated_mb = 0.0
        self.global_max_reserved_mb = 0.0
        self.tb_writer = None
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        if model is None or self.tb_writer is None or not state.is_world_process_zero:
            return control

        groups = {
            "lora_A": {"grad_sq": 0.0, "weight_sq": 0.0, "count": 0},
            "lora_route": {"grad_sq": 0.0, "weight_sq": 0.0, "count": 0},
            "lora_B": {"grad_sq": 0.0, "weight_sq": 0.0, "count": 0},
        }
        ...
        for name, param in model.named_parameters():
            ...
            groups[key]["count"] += 1
            groups[key]["weight_sq"] += param.data.float().norm(2).item() ** 2
            if param.grad is not None:
                groups[key]["grad_sq"] += param.grad.data.float().norm(2).item() ** 2

        step = state.global_step

        for key, value in groups.items():
            self.tb_writer.add_scalar(f"custom/{key}_grad_norm", value["grad_sq"] ** 0.5, step)
            self.tb_writer.add_scalar(f"custom/{key}_weight_norm", value["weight_sq"] ** 0.5, step)
            self.tb_writer.add_scalar(f"custom/{key}_param_count", value["count"], step)
        ...
        if torch.cuda.is_available():
            ...
            self.tb_writer.add_scalar("custom/gpu_memory_allocated_mb", allocated_mb, step)
            self.tb_writer.add_scalar("custom/gpu_memory_reserved_mb", reserved_mb, step)
            self.tb_writer.add_scalar("custom/gpu_memory_max_allocated_mb", max_allocated_mb, step)
            self.tb_writer.add_scalar("custom/gpu_memory_max_reserved_mb", max_reserved_mb, step)

            self.global_max_allocated_mb = max(self.global_max_allocated_mb, max_allocated_mb)
            self.global_max_reserved_mb = max(self.global_max_reserved_mb, max_reserved_mb)
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if self.tb_writer is not None and state.is_world_process_zero:
            self.tb_writer.add_scalar(
                "custom/final_gpu_global_max_memory_allocated_mb",
                self.global_max_allocated_mb,
                state.global_step,
            )
            self.tb_writer.add_scalar(
                "custom/final_gpu_global_max_memory_reserved_mb",
                self.global_max_reserved_mb,
                state.global_step,
            )
            self.tb_writer.flush()
        ...
        if torch.cuda.is_available():
            print(f"final_gpu_global_max_memory_allocated_mb = {self.global_max_allocated_mb:.2f}")
            print(f"final_gpu_global_max_memory_reserved_mb = {self.global_max_reserved_mb:.2f}")
        return control
...
def main():
    ...
    if model_args.tokenizer_name:
        ...
        logger.info(f"tokenizer.unk_token: {tokenizer.unk_token}")
        logger.info(f"tokenizer.unk_token_id: {tokenizer.unk_token_id}")
    ...
    trainer.add_callback(GradientNormLoggingCallback())
