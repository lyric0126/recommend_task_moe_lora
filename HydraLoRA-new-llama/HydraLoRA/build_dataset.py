import logging
import os
from dataclasses import dataclass
from typing import Dict, Sequence, Union, List

import datasets
import torch
from datasets import load_dataset, concatenate_datasets
import transformers


IGNORE_INDEX = -100

logger = logging.getLogger(__name__)

PROMPT_TEMPLATE = "{instruction}</s>"


def build_instruction_dataset(
    data_path: Union[List[str], str],
    tokenizer: transformers.PreTrainedTokenizer,
    max_seq_length: int,
    data_cache_dir=None,
    preprocessing_num_workers=None,
):
    def tokenization(examples):
        sources = []
        targets = []

        for instruction, input_text, output in zip(
            examples["instruction"],
            examples["input"],
            examples["output"],
        ):
            if input_text is not None and input_text != "":
                instruction = instruction + "\n" + input_text

            source = PROMPT_TEMPLATE.format_map({"instruction": instruction})
            target = f"{output}{tokenizer.eos_token}"

            sources.append(source)
            targets.append(target)

        tokenized_sources = tokenizer(sources, return_attention_mask=False)
        tokenized_targets = tokenizer(
            targets,
            return_attention_mask=False,
            add_special_tokens=False,
        )

        all_input_ids = []
        all_labels = []
        for s, t in zip(tokenized_sources["input_ids"], tokenized_targets["input_ids"]):
            if len(t) >= max_seq_length:
                s = []
                t = t[:max_seq_length]
            else:
                max_source_length = max_seq_length - len(t)
                s = s[-max_source_length:]
            input_ids = torch.LongTensor(s + t)
            labels = torch.LongTensor([IGNORE_INDEX] * len(s) + t)
            assert len(input_ids) == len(labels)
            all_input_ids.append(input_ids)
            all_labels.append(labels)

        return {
            "input_ids": all_input_ids,
            "labels": all_labels,
        }

    logging.warning("building dataset...")
    all_datasets = []

    if not isinstance(data_path, (list, tuple)):
        data_path = [data_path]

    for file in data_path:
        cache_root = data_cache_dir or str(os.path.dirname(file))
        cache_path = os.path.join(
            cache_root,
            os.path.basename(file).split(".")[0] + f"_{max_seq_length}",
        )
        os.makedirs(cache_path, exist_ok=True)

        try:
            processed_dataset = datasets.load_from_disk(cache_path)
            logger.info(f"Loaded processed dataset from cache: {cache_path}")
        except Exception:
            raw_dataset = load_dataset("json", data_files=file, cache_dir=cache_path)

            # 删除所有原始字段，避免 task_type / task_types / id 类字段残留
            remove_columns = [
                col
                for col in raw_dataset["train"].column_names
                if col not in {"input_ids", "labels"}
            ]

            tokenized_dataset = raw_dataset.map(
                tokenization,
                batched=True,
                num_proc=preprocessing_num_workers,
                remove_columns=remove_columns,
                keep_in_memory=False,
                desc="preprocessing on dataset",
            )
            processed_dataset = tokenized_dataset
            processed_dataset.save_to_disk(cache_path)
            logger.info(f"Saved processed dataset to cache: {cache_path}")

        processed_dataset.set_format(type="torch")
        train_split = processed_dataset["train"]
        logger.info(f"Dataset columns for {file}: {train_split.column_names}")
        logger.info(f"First sample keys for {file}: {list(train_split[0].keys())}")
        all_datasets.append(train_split)

    all_datasets = concatenate_datasets(all_datasets)
    logger.info(f"Final concatenated dataset columns: {all_datasets.column_names}")
    return all_datasets


@dataclass
class DataCollatorForSupervisedDataset:
    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids = [instance["input_ids"] for instance in instances]
        labels = [instance["labels"] for instance in instances]

        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids,
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        )
        labels = torch.nn.utils.rnn.pad_sequence(
            labels,
            batch_first=True,
            padding_value=IGNORE_INDEX,
        )

        batch = {
            "input_ids": input_ids,
            "labels": labels,
            "attention_mask": input_ids.ne(self.tokenizer.pad_token_id),
        }
        return batch
